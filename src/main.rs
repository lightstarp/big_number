...
fn saiki(t: Vec<Num>){
    ...
    if count == 1 {
        if t[idx].r.dividend == 63 && t[idx].r.divisor == 1 {
...
fn main() {
    let mut t: Vec<Ratinal> = vec![build_ratinal(0,1); 5];
    t[0] = build_ratinal(7,1);
    t[1] = build_ratinal(7,1);
    t[2] = build_ratinal(7,1);
    t[3] = build_ratinal(7,1);
    t[4] = build_ratinal(7,1);
